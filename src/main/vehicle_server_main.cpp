...
void printUsage(const char* program_name) {
    ...
    std::cout << "  --type <type>            Vehicle type (bus/tram/trolleybus)" << std::endl;
    std::cout << "  -p, --port <port>        Server port (default: 8081)" << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    std::string log_file = "logs/vehicle_server.log";
    bool verbose = false;
    ...
    try {
        // Initialize logger
        auto logger = Logger::getLogger("VehicleServer");
        logger->initialize(log_file, verbose ? Logger::LogLevel::DEBUG : Logger::LogLevel::INFO);
        
        logger->info("Starting Vehicle Server (" + vehicle_type + ")...");
        ...
        // Create and configure server (using VehicleServer)
        server = std::make_unique<VehicleServer>();
        ...
        // Start server
        logger->info("Starting " + vehicle_type + " server on port " + std::to_string(port) + "...");
        if (!server->start(port, config_file)) {
            logger->error("Failed to start server");
            return 1;
        }

        logger->info("Vehicle Server (" + vehicle_type + ") started successfully");
        std::cout << vehicle_type << " Server is running on port " << port << std::endl;
